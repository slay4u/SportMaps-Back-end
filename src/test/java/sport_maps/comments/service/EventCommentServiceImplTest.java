...
class EventCommentServiceImplTest {
    @Mock
    private EventCommentDao coachDao;

    @Mock
    private UserDao userDao;

    @Mock
    private EventDao forumDao;

    @Mock
    private Mapper mapper;

    @InjectMocks
    private EventCommentServiceImpl coachService;

    private EventComment coach;
    private Event forum;
    ...
    void setUp() {
        ...
        forum.setSportType(SportType.SportKite);
        coach = new EventComment();
        coach.setEvent(forum);
    ...
    void create_returnsObject() {
        ...
        lenient().when(coachDao.save(Mockito.any(EventComment.class))).thenReturn(coach);
    ...
    void update() {
        ...
        lenient().when(forumDao.findById(1L)).thenReturn(Optional.of(forum));
        lenient().when(coachDao.save(Mockito.any(EventComment.class))).thenReturn(coach);
        lenient().when(mapper.convertToEntity(Mockito.any(CommentSaveDto.class), Mockito.any(User.class), Mockito.any(Event.class))).thenReturn(coach);
        coachService.createComment(coachSaveDto);
