...
class EventDaoTest {
    ...
    public void getAll_returnsAllObjects() {
        Event comment2 = new Event();
        comment2.setText("Just good");
        comment2.setName("Bum 2024");
        comment2.setSportType(SportType.SportKite);
        comment2.setAuthor(user);
        eventDao.save(event);
        eventDao.save(comment2);
        List<Event> eventComments = eventDao.findAll();
    ...
    public void getById_returnsObject() {
        eventDao.save(event);
        Event comment1 = eventDao.findById(event.getId()).get();
    ...
    public void update_savesObject() {
        ...
        Event updated = eventDao.save(comment1);
    ...
    public void delete_deletesObject() {
        ...
        Optional<Event> optional = eventDao.findById(event.getId());
