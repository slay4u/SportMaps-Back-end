package spring.app.config.aspect;
...
import spring.app.service.auth.UserDetailsServiceImpl;
...
public class UserAccessAspect {
    ...
    @Before("execution(* spring.app.service.auth.AuthenticationService.refreshToken(..))")
    public void checkUsernameRefreshToken(JoinPoint joinPoint) {
        ...
        spring.app.domain.User userToCheck = userDetailsService
    ...
    private void checkTwoUsers(JoinPoint joinPoint) {
        ...
        spring.app.domain.User userToCheck = getUserOrThrow(idToCheck);
        compareUsers(user, userToCheck);
    }

    private spring.app.domain.User getUserOrThrow(Long idUser) {
        return userDetailsService.getUserById(idUser)
                .orElseThrow(() -> new AuthenticationException("User id not found " + idUser));
    }

    private void compareUsers(User user, spring.app.domain.User userToCheck) {
